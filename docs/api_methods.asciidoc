...
`scrollSize`::
`Integer` -- Size on the scroll request powering the update_by_query
`waitForCompletion`::
`Boolean` -- Should the request should block until the reindex is complete.
`index`::
`String, String[], Boolean` -- A comma-separated list of index names to search; use `_all` or empty string to perform the operation on all indices
...
`parentTask`::
`Number` -- Cancel tasks with specified parent task id. Set to -1 to cancel all.
`taskId`::
`Number` -- Cancel the task with specified id
...
`taskId`::
`Number` -- Return the task with specified id
